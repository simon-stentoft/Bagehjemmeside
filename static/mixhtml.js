...
function mix_convert() {
  try {
    document
      .querySelectorAll(
        "[mix-get], [mix-post], [mix-put], [mix-patch], [mix-delete], [mix-ttl]"
      )
      .forEach((el) => {
...
function process_template(mix_url) {
  try {
    ...
    if (
      !document.querySelector("template[mix-replace]") &&
    ...
    document.querySelectorAll("template[mix-replace]").forEach((template) => {
      try {
        ...
        console.log(
          `ok : mix-replace the response data will affect '${template.getAttribute(
            "mix-replace"
          )}'`
        );
        document
          .querySelector(template.getAttribute("mix-replace"))
    ...
    document.querySelectorAll("template[mix-update]").forEach((template) => {
      try {
        ...
        console.log(
          `ok : mix-update - the response data will affect '${template.getAttribute(
            "mix-update"
          )}'`
        );
        document.querySelector(template.getAttribute("mix-update")).innerHTML =
          template.innerHTML;
        if (!template.getAttribute("mix-push-url")) {
          cl(`process_template() - optional - mix-push-url not set`);
        }
    ...
    document.querySelectorAll("template[mix-top]").forEach((template) => {
      try {
        ...
        console.log(
          `ok : mix-top the response data will affect '${template.getAttribute(
            "mix-top"
          )}'`
        );
        document
          .querySelector(template.getAttribute("mix-top"))
          .insertAdjacentHTML("afterbegin", template.innerHTML);
        if (!template.getAttribute("mix-push-url")) {
          cl(`process_template() - optional - mix-push-url not set`);
        }
        mix_convert();
    ...
    document.querySelectorAll("template[mix-bottom]").forEach((template) => {
      try {
        ...
        console.log(
          `ok : mix-bottom the response data will affect '${template.getAttribute(
            "mix-bottom"
          )}'`
        );
        document
          .querySelector(template.getAttribute("mix-bottom"))
          .insertAdjacentHTML("beforeend", template.innerHTML);
        if (!template.getAttribute("mix-push-url")) {
          cl(`process_template() - optional - mix-push-url not set`);
        }
        mix_convert();
    ...
    document.querySelectorAll("template[mix-before]").forEach((template) => {
      try {
        ...
        console.log(
          `ok : mix-before the response data will affect '${template.getAttribute(
            "mix-before"
          )}'`
        );
        document
          .querySelector(template.getAttribute("mix-before"))
          .insertAdjacentHTML("beforebegin", template.innerHTML);
        if (!template.getAttribute("mix-push-url")) {
          cl(`process_template() - optional - mix-push-url not set`);
        }
        mix_convert();
    ...
    document.querySelectorAll("template[mix-after]").forEach((template) => {
      try {
        ...
        console.log(
          `ok : mix-after the response data will affect '${template.getAttribute(
            "mix-after"
          )}'`
        );
        document
          .querySelector(template.getAttribute("mix-after"))
          .insertAdjacentHTML("afterend", template.innerHTML);
        if (!template.getAttribute("mix-push-url")) {
          cl(`process_template() - optional - mix-push-url not set`);
        }
    ...
    document.querySelectorAll("template[mix-function]").forEach((template) => {
      try {
        ...
        console.log(
          `ok : mix() the response data will run the function '${function_name}'`
        );
